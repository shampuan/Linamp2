...
# GNOME / Wayland ortamında kararlı pencere dekorasyonu ve ölçekleme
...
from PyQt6.QtWidgets import (
    ...
    QFrame, QFileDialog, QSplitter, QStyle, QMessageBox, QMenu
)
from PyQt6.QtGui import QPainter, QColor, QPen, QBrush, QFont, QDragEnterEvent, QDropEvent, QIcon, QMovie, QFontDatabase, QAction, QPalette, QDesktopServices
...
SOCKET_FILE = os.path.join(CONFIG_DIR, "linamp2.sock")

def send_to_existing_instance(files):
    """Eğer açık bir Linamp2 varsa dosyaları ona gönderir, teyit bekler ve True döner."""
    ...
    try:
        ...
        client.shutdown(socket.SHUT_WR)  # Veri gönderiminin bittiğini bildir
        # Ana pencereden 'OK' teyidini bekle (böylece veri kaybolmaz)
        try:
            client.recv(1024)
        except Exception:
            pass
...
class IPCServer(QObject):
    """Açık olan pencerenin yeni tıklanan şarkıları dinlediği arka plan sunucusu."""
    ...
    def _run(self):
        try:
            ...
            while self._running:
                ...
                if raw_data:
                    try:
                        files = json.loads(raw_data.decode('utf-8'))
...
class AudioVisualizerWidget(QFrame):
    ...
    def __init__(self, gui_dir, parent=None):
        ...
        if font_id != -1:
            ...
            self.vfd_font = QFont(font_family, 8, QFont.Weight.Bold)
        else:
            custom_font = QFont("Monospace", 9, QFont.Weight.Bold)
            time_font = QFont("Monospace", 9, QFont.Weight.Bold)
            self.vfd_font = QFont("Monospace", 7, QFont.Weight.Bold)

        # Üst VFD Şerit Değişkenleri (Stereo/Mono, Değerler ve Formatlar)
        self.info_channels = 0        # 0: Kapalı, 1: Mono, 2: Stereo
        self.info_format = ""         # "mp3", "flac", "wav", "ogg", "opus" vb.
        self.info_text = "--- kbps"   # Tam kbps metni
    ...
    def set_audio_info(self, channels=0, fmt="", bitrate="--- kbps", sample="-- kHz", bits="-- bit"):
        """Üst VFD göstergesinin lamba ve metin durumlarını günceller."""
        self.info_channels = channels
        self.info_format = fmt.lower().strip()
        self.info_bitrate = bitrate
        self.info_sample = sample
        self.info_bits = bits
        self.update()
    ...
    def paintEvent(self, event):
        ...
        # ----------------------------------------------------------------------
        # 1.5 İKİ KATLI VFD TEYP EKRANI (ÜSTTE SABİT PARAMETRELER, ALTTA FORMATLAR)
        # ----------------------------------------------------------------------
        ...
        vfd_font = QFont("Sans", 7, QFont.Weight.Bold)
        painter.setFont(vfd_font)
        fm = painter.fontMetrics()

        # İki Satırın Dikey Konumları
        row1_y = 11                           # 1. Satır: Stereo/Mono, kbps, kHz
        row2_y = 22                           # 2. Satır: Format Rozetleri Skalası

        # Renk Paleti
        c_dim = QColor(39, 46, 43)          # Açık ve net görünen antrasit kılavuz
        c_green = QColor(0, 255, 148)       # Neon Yeşil
        c_red = QColor(255, 56, 56)         # Retro Kırmızı
        c_yellow = QColor(255, 205, 0)      # Parlak Altın Sarısı
        c_blue = QColor(30, 110, 240)       # Mavi ya da yeşil karar veremedim bakıcaz

        # ======================================================================
        # 1. SATIR: SABİT SÜTUNLU SES PARAMETRELERİ (Asla Sağa Sola Kaymaz)
        # ======================================================================
        # Sütun 1: STEREO / MONO (Sol)
        st_col = c_green if self.info_channels == 2 else c_dim
        mo_col = c_red if self.info_channels == 1 else c_dim
        painter.setPen(st_col)
        painter.drawText(spec_x, row1_y, "STEREO")
        painter.setPen(mo_col)
        painter.drawText(spec_x + 48, row1_y, "MONO")

        # Sütun 2: KILOBIT DEĞERİ (Orta - Sarı)
        br_text = getattr(self, 'info_bitrate', "--- kbps")
        val_col = c_yellow if self.info_channels > 0 else c_dim
        painter.setPen(val_col)
        painter.drawText(spec_x + 92, row1_y, br_text)

        # Sütun 3: SAMPLE RATE (Yeşil)
        sr_text = getattr(self, 'info_sample', "-- kHz")
        sr_col = c_green if self.info_channels > 0 else c_dim
        painter.setPen(sr_col)
        painter.drawText(spec_x + 142, row1_y, sr_text)

        # Sütun 4: BİT DERİNLİĞİ (Sarı - 16-bit / 24-bit)
        bit_text = getattr(self, 'info_bits', "-- bit")
        bit_col = c_yellow if self.info_channels > 0 else c_dim
        painter.setPen(bit_col)
        painter.drawText(spec_x + 185, row1_y, bit_text)

        # ======================================================================
        # 2. SATIR: TÜM FORMAT ROZETLERİ (Geniş Geniş Boylu Boyunca)
        # ======================================================================
        all_badges = ["MP3", "FLAC", "WAV", "OGG", "OPUS", "M4A", "AAC", "WMA"]
        cur_fmt_x = spec_x
        for name in all_badges:
            if self.info_format == name.lower():
                badge_col = c_blue
            elif self.info_format == "alac" and name == "M4A":
                badge_col = c_blue
            elif self.info_format == "oga" and name == "OGG":
                badge_col = c_blue
            else:
                badge_col = c_dim

            painter.setPen(badge_col)
            painter.drawText(cur_fmt_x, row2_y, name)
            cur_fmt_x += fm.horizontalAdvance(name) + 6
...
class DropPlaylistWidget(QListWidget):
    ...
    def dropEvent(self, event: QDropEvent):
        ...
        win = self.window()
        if files and hasattr(win, "add_files_to_playlist"):
            win.add_files_to_playlist(files)
...
class Linamp2Window(QMainWindow):
    def __init__(self):
        super().__init__()
        # Temel Değişkenler (Çökme ve AttributeError önleyicileri)
        self.analyzer = None
        ...
        # 1. Dosya Yöneticisinden (FM) çift tıklanan dosyaları baştan ayrıştır ve normalize et
        from urllib.parse import unquote, urlparse
        self.cli_files = []
        for arg in sys.argv[1:]:
            clean_arg = arg
            if clean_arg.startswith("file://"):
                clean_arg = unquote(urlparse(clean_arg).path)
            if os.path.exists(clean_arg):
                self.cli_files.append(os.path.abspath(clean_arg))

        # 2. Ayarları JSON dosyasından oku
        ...
        # Kaydedilmiş ayarları arayüze uygula (Ses, EQ, Playlist)
        ...
        # Arka plandan gelecek yeni dosya tıklamalarını dinle (IPC)
        self.ipc_server = IPCServer(SOCKET_FILE)
        self.ipc_server.files_received.connect(self.handle_external_files)

        # İlk açılışta çift tıklanan dosya varsa başlat
        if self.cli_files:
            self.clear_playlist()
            self.add_files_to_playlist(self.cli_files)
            if self.playlist_data:
                QTimer.singleShot(100, lambda: self.play_index(0))

        # Görselleştirme ve Arayüz Güncelleme Zamanlayıcısı (30 FPS)
        self.timer = QTimer(self)
        self.timer.setInterval(33)
        self.timer.timeout.connect(self.update_ui_loop)
        self.timer.start()

    def handle_external_files(self, files):
        """Pencere açıkken FM'den yeni bir şarkıya çift tıklandığında içeriği sıfırlar ve çalar."""
        if files:
            self.clear_playlist()
            self.add_files_to_playlist(files)
            if self.playlist_data:
                QTimer.singleShot(50, lambda: self.play_index(0))
        self.showNormal()
        self.raise_()
        self.activateWindow()
    ...
    def init_ui(self):
        ...
        self.btn_repeat.clicked.connect(self.save_settings)
        ...
        self.btn_shuffle.clicked.connect(self.save_settings)

        # Ekolayzır Paneli Göster/Gizle Butonu
        ...
        self.btn_toggle_eq.setToolTip(_("Show/Hide Equalizer Panel"))
        ...
        # Sol Kontrol Sütunu (ON/OFF ve 0 dB Sıfırla)
        eq_ctrl_box = QVBoxLayout()
        eq_ctrl_box.setSpacing(4)
        eq_ctrl_box.setAlignment(Qt.AlignmentFlag.AlignCenter)

        self.btn_eq_power = QPushButton(_("ON"))
        self.btn_eq_power.setCheckable(True)
        self.btn_eq_power.setChecked(True)
        self.btn_eq_power.setFixedSize(38, 36)
        self.btn_eq_power.setCursor(Qt.CursorShape.PointingHandCursor)
        self.btn_eq_power.setToolTip(_("Enable/Disable Equalizer"))
        self.btn_eq_power.setStyleSheet("QPushButton { color: #00ff94; background: #222; border: 1px solid #00ff94; font-weight: bold; border-radius: 3px; font-size: 10px; }")
        self.btn_eq_power.clicked.connect(self.on_eq_power_toggled)

        self.btn_eq_zero = QPushButton(_("0dB"))
        self.btn_eq_zero.setFixedSize(38, 24)
        self.btn_eq_zero.setCursor(Qt.CursorShape.PointingHandCursor)
        self.btn_eq_zero.setToolTip(_("Reset all sliders to 0 dB"))
        self.btn_eq_zero.setStyleSheet("QPushButton { color: #aaaaaa; background: #2a2a2a; border: 1px solid #444; font-size: 9px; border-radius: 3px; }")
        self.btn_eq_zero.clicked.connect(self.reset_equalizer_bands)

        eq_ctrl_box.addWidget(self.btn_eq_power)
        eq_ctrl_box.addWidget(self.btn_eq_zero)
        eq_layout.addLayout(eq_ctrl_box)
        ...
        # Çoklu seçim (Ctrl+A, Shift ve fareyle birden fazla şarkı seçebilme) izni
    ...
    def apply_loaded_settings(self):
        ...
        # 3. Ekolayzır Paneli ve Filtre Açık/Kapalı Durumları
        eq_visible = s.get("equalizer_visible", True)
        self.btn_toggle_eq.setChecked(eq_visible)
        self.toggle_equalizer()

        eq_enabled = s.get("equalizer_enabled", True)
        self.btn_eq_power.setChecked(eq_enabled)
        self.on_eq_power_toggled()
        ...
        # 5. En Son Çalma Listesini Yükle (Eğer çift tıklanan dosya yoksa)
        if not getattr(self, 'cli_files', None):
    ...
    def save_settings(self):
        """Mevcut durumları anında ~/.config/Linamp2/settings.json dosyasına yazar."""
        ...
        try:
            ...
            data = {
                ...
                "equalizer_visible": self.btn_toggle_eq.isChecked(),
                "equalizer_enabled": self.btn_eq_power.isChecked(),
    ...
    def create_menu_bar(self):
        ...
        act_select_all.triggered.connect(lambda: self.playlist_widget.selectAll())
        edit_menu.addAction(act_select_all)

        act_remove_sel = QAction(_("&Remove Selected"), self)
        act_remove_sel.setShortcut("Delete")
        act_remove_sel.triggered.connect(lambda: self.remove_selected_playlist_items())
    ...
    def add_files_to_playlist(self, file_paths):
        # Opus ve türevleri (.oga, .webm sesleri dahil) tüm yaygın formatlar
        valid_exts = ('.mp3', '.flac', '.wav', '.ogg', '.oga', '.opus', '.m4a', '.aac', '.wma', '.webm')
        added = False
        if not hasattr(self, '_item_counter'):
            self._item_counter = 0

        for path in file_paths:
            clean_path = str(path).strip().strip("'").strip('"')
            if clean_path.lower().endswith(valid_exts):
                ...
                self.playlist_data.append({
                    'path': clean_path,
                    'title': file_name,
                    'added_id': self._item_counter
                })
                self._item_counter += 1
                added = True
                print(f"[Linamp2] Listeye başarıyla eklendi: {file_name}")
            else:
                print(f"[Linamp2] Dosya reddedildi (Uzantı desteklenmiyor): {clean_path}")
        if added:
            self.save_settings()
    ...
    def open_file_dialog(self):
        ...
        files, _filter = QFileDialog.getOpenFileNames(
            self, _("Select Audio Files"), home_dir, _("Audio Files (*.mp3 *.flac *.wav *.ogg *.m4a *.opus)")
        )
        if files:
            self.add_files_to_playlist(files)
    ...
    def open_playlist_dialog(self):
        ...
        home_dir = os.path.expanduser("~")
        pl_path, _filter = QFileDialog.getOpenFileName(
    ...
    def save_playlist_dialog(self):
        ...
        home_dir = os.path.expanduser("~")
        pl_path, _filter = QFileDialog.getSaveFileName(
    ...
    def clear_playlist(self):
        ...
        self.visualizer.set_audio_info(0, "", "--- kbps", "-- kHz", "-- bit")
        self.save_settings()

    def extract_audio_properties(self, file_path):
        """Parçanın format, kanal, bitrate, sample rate ve bit derinliği bilgilerini çeker."""
        ext = os.path.splitext(file_path)[1].lower().lstrip('.')
        channels = 2
        bitrate_str = "--- kbps"
        sample_str = "-- kHz"
        bit_str = "16-bit"  # Standart çözünürlük yedeği
        try:
            ...
            if audio and hasattr(audio, 'info') and audio.info:
                ...
                if br:
                    bitrate_str = f"{int(round(br / 1000))} kbps"
                elif length > 0 and os.path.exists(file_path):
                    file_size_bits = os.path.getsize(file_path) * 8
                    calc_kbps = int(round(file_size_bits / length / 1000))
                    bitrate_str = f"{calc_kbps} kbps"

                # 2. Bit Derinliği (Sadece FLAC/WAV gibi gerçek PCM dosyalarında parlar)
                bps = getattr(audio.info, 'bits_per_sample', 0)
                if bps:
                    bit_str = f"{bps}-bit"
                else:
                    bit_str = "-- bit"

                # 3. Örnekleme Hızı (Opus standardı 48 kHz'dir)
                sr = getattr(audio.info, 'sample_rate', 0) or getattr(audio.info, 'samplerate', 0)
                if not sr and ext == "opus":
                    sr = 48000
                if sr:
                    sr_k = sr / 1000.0
                    sample_str = f"{sr_k:.0f} kHz" if sr_k.is_integer() else f"{sr_k:.1f} kHz"
        except Exception:
            pass

        return channels, ext, bitrate_str, sample_str, bit_str
            ...
            if hasattr(audio, 'tags') and audio.tags:
                for tag in audio.tags.values():
                    if getattr(tag, 'FrameID', None) == 'APIC':
                        return tag.data
            # FLAC ve Ogg/Opus Picture bloğu
            if hasattr(audio, 'pictures') and audio.pictures:
                return audio.pictures[0].data
            # Opus / Ogg metadata_block_picture
    ...
    def play_index(self, index):
        if 0 <= index < len(self.playlist_data):
            ...
            try:
                ...
                print(f"[Linamp2] Oynatma hatası ({item_info['path']}): {e}")
                return
            ...
            # Üst VFD lambalarını (Stereo, Format, kbps, kHz ve bit) ateşle
            ch, fmt, br_str, sr_str, bit_str = self.extract_audio_properties(item_info['path'])
            self.visualizer.set_audio_info(ch, fmt, br_str, sr_str, bit_str)
    ...
    def stop_music(self):
        ...
        self.visualizer.set_audio_info(0, "", "---k --k")
    ...
    def show_playlist_context_menu(self, pos):
        """Çalma listesi öğesine veya boşluğa sağ tıklandığında açılacak menü."""
        item = self.playlist_widget.itemAt(pos)
        menu = QMenu(self)

        # 1. Bir şarkı üzerine sağ tıklandıysa öğeye özel işlemler
        if item:
            row = self.playlist_widget.row(item)
            act_remove = QAction(_("Remove from Playlist"), self)
            act_remove.triggered.connect(lambda: self.remove_item_at_index(row))
            menu.addAction(act_remove)

            act_open_folder = QAction(_("Open Containing Folder"), self)
            act_open_folder.triggered.connect(lambda: self.open_containing_folder(row))
            menu.addAction(act_open_folder)

            menu.addSeparator()

        # 2. Sıralama (Sort) Alt Menüsü
        sort_menu = menu.addMenu(_("Sort Playlist"))

        act_sort_title = QAction(_("By Title / Name"), self)
        act_sort_title.triggered.connect(lambda: self.sort_playlist("title"))
        sort_menu.addAction(act_sort_title)

        act_sort_artist = QAction(_("By Artist"), self)
        act_sort_artist.triggered.connect(lambda: self.sort_playlist("artist"))
        sort_menu.addAction(act_sort_artist)

        act_sort_album = QAction(_("By Album"), self)
        act_sort_album.triggered.connect(lambda: self.sort_playlist("album"))
        sort_menu.addAction(act_sort_album)

        act_sort_track = QAction(_("By Track Number"), self)
        act_sort_track.triggered.connect(lambda: self.sort_playlist("track"))
        sort_menu.addAction(act_sort_track)

        act_sort_file = QAction(_("By File Name"), self)
        act_sort_file.triggered.connect(lambda: self.sort_playlist("filename"))
        sort_menu.addAction(act_sort_file)

        sort_menu.addSeparator()

        act_sort_original = QAction(_("Reset to Original Order"), self)
        act_sort_original.triggered.connect(lambda: self.sort_playlist("original"))
        sort_menu.addAction(act_sort_original)

        # Liste boşsa sıralama menüsünü pasif yap
        if not self.playlist_data:
    ...
    def extract_sort_tags(self, file_path):
        """Dosyanın ID3 / Vorbis / FLAC / MP4 etiketlerinden sıralama verilerini ayıklar."""
        ...
        try:
            # easy=True parametresi MP3, FLAC, OGG, Opus ve MP4 için standart etiketler sunar
            ...
            if audio:
                ...
                if 'tracknumber' in audio and audio['tracknumber']:
                    raw_tr = str(audio['tracknumber'][0]).split('/')[0].strip()
                    try:
                        track_num = int(raw_tr)
                    except ValueError:
                        track_num = 0
        except Exception:
            pass
    ...
    def sort_playlist(self, sort_by):
        """Çalma listesini belirtilen kritere göre sıralar ve çalan şarkının indeksini korur."""
        if not self.playlist_data:
            return

        # 1. Çalmakta olan şarkının yolunu sakla (İndeks kaymasını önlemek için)
        current_path = None
        if 0 <= self.current_index < len(self.playlist_data):
            current_path = self.playlist_data[self.current_index]['path']

        # 2. Tüm parçaların sıralama bilgilerini önbelleğe al
        tags_cache = {item['path']: self.extract_sort_tags(item['path']) for item in self.playlist_data}

        # 3. Kriterlere göre akıllı sıralama yap
        if sort_by == "title":
            self.playlist_data.sort(key=lambda x: tags_cache[x['path']]['title'])
        elif sort_by == "artist":
            # Sanatçı -> Albüm -> Parça Numarası
            self.playlist_data.sort(key=lambda x: (
                tags_cache[x['path']]['artist'],
                tags_cache[x['path']]['album'],
                tags_cache[x['path']]['track'],
                tags_cache[x['path']]['title']
            ))
        elif sort_by == "album":
            # Albüm -> Parça Numarası -> Şarkı Adı
            self.playlist_data.sort(key=lambda x: (
                tags_cache[x['path']]['album'],
                tags_cache[x['path']]['track'],
                tags_cache[x['path']]['title']
            ...
            # Parça Numarası -> Şarkı Adı
            self.playlist_data.sort(key=lambda x: (
                tags_cache[x['path']]['track'],
                tags_cache[x['path']]['title']
            ))
        elif sort_by == "filename":
            self.playlist_data.sort(key=lambda x: tags_cache[x['path']]['filename'])
        elif sort_by == "original":
            # Listeye eklenme sırasına (added_id) göre geri sırala
            self.playlist_data.sort(key=lambda x: x.get('added_id', 0))

        # 4. Arayüzdeki (QListWidget) listeyi güncelle
        self.playlist_widget.clear()
        for item in self.playlist_data:
            self.playlist_widget.addItem(QListWidgetItem(item['title']))

        # 5. Çalan şarkının yeni sıradaki yerini tespit et ve seçili yap
        if current_path:
        ...
        # 6. Yeni liste sırasını settings.json'a kaydet
        self.save_settings()

    def remove_item_at_index(self, row):
        """Seçili parçayı listeden kaldırır ve çalan şarkı indeksini günceller."""
        if 0 <= row < len(self.playlist_data):
    ...
    def remove_selected_playlist_items(self):
        """Listede seçili olan tüm öğeleri indeks kaymalarını önleyerek sondan başa siler."""
        selected_rows = sorted([self.playlist_widget.row(item) for item in self.playlist_widget.selectedItems()], reverse=True)
        for row in selected_rows:
            self.remove_item_at_index(row)

    def open_containing_folder(self, row):
        """Parçanın bulunduğu klasörü sistemin varsayılan dosya yöneticisinde açar."""
        if 0 <= row < len(self.playlist_data):
            file_path = self.playlist_data[row]['path']
    ...
    def closeEvent(self, event):
        ...
        if analyzer:
            analyzer.stop()
        capture_proc = getattr(self, 'capture_proc', None)
        if capture_proc:
            try:
    ...
    def toggle_equalizer(self):
        """Sadece ekolayzır panelini gösterir veya gizler (Sesi etkilemez)."""
    ...
    def on_eq_power_toggled(self):
        """Panelin içindeki ON/OFF butonuna basıldığında filtreyi açar veya kapatır."""
        is_on = self.btn_eq_power.isChecked()
        if is_on:
            self.btn_eq_power.setText(_("ON"))
            self.btn_eq_power.setStyleSheet("QPushButton { color: #00ff94; background: #222; border: 1px solid #00ff94; font-weight: bold; border-radius: 3px; font-size: 10px; }")
        else:
            self.btn_eq_power.setText(_("OFF"))
            self.btn_eq_power.setStyleSheet("QPushButton { color: #888888; background: #222; border: 1px solid #555; font-weight: bold; border-radius: 3px; font-size: 10px; }")
        
        self.apply_equalizer()
        self.save_settings()

    def reset_equalizer_bands(self):
        """Tüm ekolayzır sürgülerini 0 dB seviyesine çeker."""
    ...
    def apply_equalizer(self):
        ...
        # Eğer panel içi ON/OFF butonu kapalıysa filtreyi temizle (Bypass/Saf ses)
        if not getattr(self, 'btn_eq_power', None) or not self.btn_eq_power.isChecked():
            try:
                self.player.af = ""
            except Exception:
                pass
    ...
    def update_ui_loop(self):
        ...
        if self.is_playing:
            try:
                ...
                # MPV dosyayı yüklerken sahte eof_reached vermesin diye pos kontrolü yapılır
                if pos is not None and pos > 0.1 and getattr(self.player, 'eof_reached', False):
...
if __name__ == "__main__":
    ...
    for arg in sys.argv[1:]:
        ...
        if clean_arg.startswith("file://"):
            clean_arg = unquote(urlparse(clean_arg).path)
        if os.path.exists(clean_arg):
            startup_files.append(os.path.abspath(clean_arg))

    # 2. Eğer zaten açık bir Linamp2 varsa, dosyaları ona gönder ve derhal çık!
    if send_to_existing_instance(startup_files):
        sys.exit(0)
    ...
    # GNOME Wayland oturumunda dock ve panel simgesi için XDG masaüstü kimliği
    app.setDesktopFileName("linamp2")
